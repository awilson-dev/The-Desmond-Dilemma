...
public extension Renderer {
    mutating func draw(_ world: World) {
        ...
        let fieldOfView = 2 * atan(viewWidth * 0.5 / focalLength)
        ...
        for x in 0 ..< columns {
            ...
            //Draw sky
            if world.needsSkyTexture {
                let skyTexture = textures[world.map.sky]
                let amountOfTexture = fieldOfView * 1 / .pi
                let playerDirection = world.player.direction.angle
                let textureX = Double(x) * Double(skyTexture.width) / Double(bitmap.width) * amountOfTexture + playerDirection * Double(skyTexture.width) * 1 / .pi
                let scale = Double(bitmap.height) / Double(skyTexture.height) + 1
                bitmap.drawColumn(Int(textureX), of: skyTexture, at: Vector(x: Double(x), y: -scale), height: Double(bitmap.height) + scale * 2)
            }
            ...
            bitmap.drawColumn(textureX, of: wallTexture, at: wallStart, height: height, distance: wallDistance, fadeStrength: world.map.distanceFade)
            
            // Draw switch
            if let s = world.switch(at: tileX, tileY) {
                let switchTexture = textures[s.animation.texture]
                bitmap.drawColumn(textureX, of: switchTexture, at: wallStart, height: height, distance: wallDistance, fadeStrength: world.map.distanceFade)
            }
            
            // Draw fans
            if let fan = world.fan(at: tileX, tileY) {
                let fanTexture = textures[fan.animation.texture]
                bitmap.drawColumn(textureX, of: fanTexture, at: wallStart, height: height, distance: wallDistance, fadeStrength: world.map.distanceFade)
            }
            ...
            for y in min(floorStart, bitmap.height) ..< bitmap.height {
                ...
                bitmap[x, y] = floorTexture[normalized: textureX, textureY].darken(distance: distance, strength: world.map.distanceFade)
                //                bitmap[x, bitmap.height - 1 - y] = ceilingTexture[normalized: textureX, textureY]
                bitmap.blendPixel(at: x, bitmap.height - 1 - y, with: ceilingTexture[normalized: textureX, textureY].darken(distance: distance, strength: world.map.distanceFade))
            }
            ...
            for (hit, spriteDistance, sprite) in spritesByDistance {
                ...
                bitmap.drawColumn(textureX, of: spriteTexture, at: start, height: height, distance: spriteDistance, fadeStrength: world.map.distanceFade)
            }
