...
public extension World {
    ...
    var needsSkyTexture: Bool {
        map.tiles.contains(where: { $0.needsSkyTexture })
    }
    ...
    mutating func reset() {
        ...
        for y in 0 ..< map.height {
            for x in 0 ..< map.width {
                ...
                case .fan:
                    precondition(map[x, y].isWall, "Fan must be placed on a wall tile")
                    fans.append(Fan(position: position, soundChannel: soundChannel))
                    soundChannel += 1
                }
            }
        }
    ...
    func fan(at x: Int, _ y: Int) -> Fan? {
        guard map.things[y * map.width + x] == .fan else {
            return nil
        }
        return fans.first(where: {
            Int($0.position.x) == x && Int($0.position.y) == y
        })
    }
}
